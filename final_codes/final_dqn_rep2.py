...
class OfflineDataCollector:
    ...
    def collect_from_frames(self, frames):
        ...
        for idx in range(len(frames) - 1):
            ...
            action = next_lane_state  # 다음 프레임의 차선 상태
...
# -----------------------------
# DQNAgentCaSSLe (rep_loss로 aug level 조절 v2)
# -----------------------------
class DQNAgentCaSSLe:
    ...
    def adaptive_aug_level_from_rep(self, rep_loss, min_level=0.05, max_level=0.5):
        """
        rep_loss가 클수록 augmentation level을 낮춘다.
        level = min + (max-min) * (1 - clamp(rep_loss,0,1))
        """
        rep_norm = torch.clamp(rep_loss.detach()*100, 0, 1)
    ...
    def train_offline(
        ...
        for epoch in range(epochs):
            ...
            # ===== 2) CaSSLe representation =====
            #   rep_loss로 augmentation level을 조절해야 하므로, 약한 augmentation으로 임시 rep_loss를 먼저 추정
            with torch.no_grad():
                aug1_tmp = self.aug(states, level=0.05)
                aug2_tmp = self.aug(states, level=0.05)
                z_online_tmp = self.policy_net(aug1_tmp, return_features=True)
                z_target_tmp = self.target_net(aug2_tmp, return_features=True)
                rep_loss_tmp = 1 - cos(z_online_tmp, z_target_tmp).mean()

            level = self.adaptive_aug_level_from_rep(rep_loss_tmp)  # rep 기반 level
            ...
            if epoch % 10 == 0:
                print(
                    f"[Offline] Epoch {epoch:03d} | loss={loss.item():.4f} | td={td_loss.item():.3f} | rep={rep_loss.item():.4f} | aug={float(level):.3f}"
                )
...
def compute_cl_metrics_partial(R_rows, R_star, upto_i):
    """
    R_rows: [R[0,:], R[1,:], ..., R[i,:]] (list of np.array shape (T,))
