...
class DQNAgent:
    ...
    def save_params_and_fisher(self, dataset, task_name, sample_max=1024):
        ...
        states_sample = torch.tensor([x[0] for x in dataset], dtype=torch.float32).to(
            self.device
        )
        actions_sample = torch.tensor([x[1] for x in dataset], dtype=torch.long).to(
            self.device
        )
...
task_frames_map = {
    ...
    "task3": frames_for_task(video_files_t3, max_frames=None),  # ← 오탈자 수정
}
...
for run_idx in range(num_runs):
    ...
    print(
        f"[After Task 1] Acc per task: {[f'{v:.3f}' for v in R1]} | FWT={m1['FWT']:.3f}, BWT={m1['BWT']:.3f}, IM={m1['IM']:.3f}"
    )

    # Task 4
    print("\n--- Training on Task 4 ---")
    s, a, r, ns, d = train_datasets["task4"]
    agent.train_offline(s, a, r, ns, d, epochs=20, beta=5, keep_buffer=False)
    agent.save_params_and_fisher(list(zip(s, a, r, ns, d)), "task4")
    R2 = evaluate_all_tasks(agent, train_order, task_frames_map, device)
    R_rows.append(R2)
    m2 = compute_cl_metrics_partial(R_rows, R_star, upto_i=2)
    print(
        f"[After Task 4] Acc per task: {[f'{v:.3f}' for v in R2]} | FWT={m2['FWT']:.3f}, BWT={m2['BWT']:.3f}, IM={m2['IM']:.3f}"
    )

    # Task 5
    print("\n--- Training on Task 5 ---")
    s, a, r, ns, d = train_datasets["task5"]
    agent.train_offline(s, a, r, ns, d, epochs=20, beta=5, keep_buffer=False)
    agent.save_params_and_fisher(list(zip(s, a, r, ns, d)), "task5")
    R3 = evaluate_all_tasks(agent, train_order, task_frames_map, device)
    R_rows.append(R3)
    m3 = compute_cl_metrics_partial(R_rows, R_star, upto_i=3)
    print(
        f"[After Task 5] Acc per task: {[f'{v:.3f}' for v in R3]} | FWT={m3['FWT']:.3f}, BWT={m3['BWT']:.3f}, IM={m3['IM']:.3f}"
    )
    ...
    print(
        f"[After Task 3] Acc per task: {[f'{v:.3f}' for v in R4]} | FWT={m4['FWT']:.3f}, BWT={m4['BWT']:.3f}, IM={m4['IM']:.3f}"
    )

    # ---- (C) test.mov: Accuracy만 ----
    acc_only, _ = evaluate_agent(agent, eval_video_file, run_index=run_idx)
    print(
        f"=== [Run {run_idx+1}] test.mov Accuracy: {acc_only:.3f} ({acc_only*100:.1f}%) ==="
    )
...
if all_accuracies:
    ...
    print(
        f"Average test.mov Accuracy: {mean_accuracy:.3f} (Std Dev: {std_accuracy:.3f})"
    )
