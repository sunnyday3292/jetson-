...
class DQNAgent:
    ...
    def adaptive_aug_level(self, rep_loss, min_level=0.05, max_level=0.3):   #max_level 0.3에서 0.5로 수정
        # [0,1] 사이로 rep_loss 정규화 (더 smooth한 정규화)
        rep_norm = torch.clamp(rep_loss.detach() * 50, 0, 1)  # 100 -> 50으로 조정
