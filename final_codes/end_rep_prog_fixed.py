...
class DQNAgent:
    ...
    def increasing_aug_level(self, epoch, total_epochs, min_level=0.05, max_level=0.3):  #max_level 0.3에서 0.5로 수정
        # 0~1 사이로 정규화
        progress = min(epoch / (total_epochs * 0.8), 1.0)  # 80% 지점에서 max 도달  #0.7에서 0.8로 수정
        # 선형 증가
        level = min_level + (max_level - min_level) * progress
        return level
