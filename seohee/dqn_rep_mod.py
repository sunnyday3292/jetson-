...
class DQN(nn.Module):
    ...
    def forward(self, x, return_features=False):
        feat = self.fc(x)
        if return_features:
            return feat  # Feature만 반환 (CaSSLe용)
        return self.q_head(feat)  # Q값 반환
...
class DQNAgentCaSSLe:
    ...
    def aug(self, x, level=0.1):
        """고급 Data Augmentation: 도메인 특화 + 적응적 노이즈"""
        x_aug = x.clone()
        
        # 1. 도메인 특화 augmentation
        # 차선 상태에 불확실성 추가 (실제 운전에서 발생하는 노이즈)
        x_aug[:, 0] += 0.15 * torch.randn(x.shape[0], device=x.device)  # 차선 감지 불확실성
        
        # 빨간색 비율에 센서 노이즈 추가
        x_aug[:, 3] += 0.05 * torch.randn(x.shape[0], device=x.device)  # 센서 노이즈
        
        # 2. 적응적 가우시안 노이즈
        adaptive_level = level * (1 + 0.2 * torch.randn(1, device=x.device).item())
        x_aug += adaptive_level * torch.randn_like(x)
        
        # 3. 경계값 클리핑으로 안정성 확보
        x_aug[:, 0] = torch.clamp(x_aug[:, 0], 0, 1)  # 차선 상태
        x_aug[:, 1] = torch.clamp(x_aug[:, 1], 0, 1)  # over_line
        x_aug[:, 2] = torch.clamp(x_aug[:, 2], 0, 1)  # red_side
        x_aug[:, 3] = torch.clamp(x_aug[:, 3], 0, 1)  # red_ratio
    ...
    def train_offline(self, state_list, action_list, reward_list, next_state_list, done_list, 
        ...
        for epoch in range(epochs):
            ...
            cql_penalty = (logsumexp_q - mean_q).mean()
            entropy_penalty = all_q.var(dim=1).mean()
            td_loss = F.mse_loss(current_q, target_q)
            
            # 2. CaSSLe: Representation Loss
            # Augmentation 2개 생성
            aug1 = self.aug(states)
            aug2 = self.aug(states)
            ...
            # 개선된 SSL Loss: 정규화된 Cosine Similarity
            z_online_norm = F.normalize(z_online, dim=1)
            z_target_norm = F.normalize(z_target, dim=1)
            rep_loss = 1 - (z_online_norm * z_target_norm).sum(dim=1).mean()
