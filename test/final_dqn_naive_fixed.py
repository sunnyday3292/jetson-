...
class LaneDetector:
    ...
    def process_frame(self, frame):
        ...
        if left_count > right_count and left_count > 0:
            lane_state = 1  # "right"로 가야 한다고 해석 (차선이 왼쪽에 있음)
...
class DQNAgent:
    def __init__(self, state_dim, action_dim=2, device='cpu',
                 cql_alpha=10, lr=1e-5, gamma=0.99, batch_size=32,
...
def evaluate_agent_on_frames(agent, frames):
    """현재 에이전트를 주어진 프레임 시퀀스(영상 1개)에 대해 정확도 계산"""
...
# -----------------------------
# STL(단일 태스크 기준선) 학습 & 평가
# -----------------------------
def single_task_baseline_accuracy(task_data, eval_frames,
    ...
    acc = evaluate_agent_on_frames(agent, eval_frames)
    return acc
...
# 태스크 구성 (순서 중요)
tasks = [
    ("T1", (s1, a1, r1, ns1, d1), video_files_t1),
    ("T4", (s4, a4, r4, ns4, d4), video_files_t4),
    ("T5", (s5, a5, r5, ns5, d5), video_files_t5),
    ("T3", (s3, a3, r3, ns3, d3), video_files_t3),
]
T = len(tasks)

# 각 태스크의 평가 프레임(여기서는 각 태스크 첫 영상 사용)
eval_frames_per_task = []
for name, _, vids in tasks:
    if len(vids) == 0:
        eval_frames_per_task.append([])
    else:
        eval_frames_per_task.append(load_video_frames(vids[0]))
...
for run in range(num_runs):
    ...
    # 성능 행렬 R: shape = (T+1, T)
    # R[0, j] = 무학습 초기 성능 (pre-training)
    # R[i, j] = i번째 태스크까지 학습 후 태스크 j 성능 (i=1..T)
    R = np.zeros((T+1, T), dtype=np.float32)

    # 0단계(무학습) 성능
    for j in range(T):
        acc0 = evaluate_agent_on_frames(agent, eval_frames_per_task[j])
        R[0, j] = acc0
    print("[Stage 0] Pre-training accuracies per task:", [f"{x:.3f}" for x in R[0]])

    # 연속 학습 진행
    for i, (name_i, data_i, vids_i) in enumerate(tasks, start=1):
        print(f"\n--- Training on {name_i} ---")
        s_i, a_i, r_i, ns_i, d_i = data_i
        agent.train_offline(s_i, a_i, r_i, ns_i, d_i, epochs=20, beta=5, keep_buffer=True)

        # i단계 학습 후, 모든 태스크 평가
        for j in range(T):
            acc_ij = evaluate_agent_on_frames(agent, eval_frames_per_task[j])
            R[i, j] = acc_ij
        print(f"[Stage {i}] Accuracies per task:", [f"{x:.3f}" for x in R[i]])

    # CL 지표 계산
    # FWT: 평균 (R[j-1, j] - R[0, j]) for j=1..T-1 (0-index로는 j=1..T-1)
    fwt_terms = []
    for j in range(1, T):
        fwt_terms.append(R[j, j] - R[0, j])  # j-1이 아니라 j로 보일 수 있지만, 위에서 Stage i==j 후 평가라 R[j, j]가 "그 태스크 학습 직후"
        # 엄밀 정의대로라면 "그 태스크 학습 전 직전 단계" R[j-1, j]을 쓰는 버전도 있음.
        # 원정의 사용하려면 다음 줄로 교체:
        # fwt_terms.append(R[j-1, j] - R[0, j])
    FWT_value = float(np.mean(fwt_terms)) if len(fwt_terms) > 0 else 0.0

    # BWT: 평균 (R[T, j] - R[j, j]) for j=0..T-2
    bwt_terms = []
    for j in range(0, T-1):
        bwt_terms.append(R[T, j] - R[j, j])
    BWT_value = float(np.mean(bwt_terms)) if len(bwt_terms) > 0 else 0.0

    # IM: 각 태스크 단일학습(STL) 기준과의 차이 평균
    STL = []
    for j in range(T):
        stl_acc = single_task_baseline_accuracy(tasks[j][1], eval_frames_per_task[j],
                                                lr=1e-5, gamma=0.99, cql_alpha=30,
                                                batch_size=32, target_update_freq=10,
                                                epochs=20)
        STL.append(stl_acc)
    STL = np.array(STL, dtype=np.float32)

    # IM = mean(STL_j - R[j, j])  (j는 0..T-1)
    im_terms = []
    for j in range(T):
        im_terms.append(STL[j] - R[j, j])
    IM_value = float(np.mean(im_terms)) if len(im_terms) > 0 else 0.0

    print("\n=== CL Metrics (this run) ===")
    print(f"FWT: {FWT_value:.4f} | BWT: {BWT_value:.4f} | IM: {IM_value:.4f}")

    all_FWT.append(FWT_value)
    all_BWT.append(BWT_value)
    all_IM.append(IM_value)

    # 기존 단일 평가(보고용): eval_video_file
    accuracy_eval = evaluate_on_video(agent, eval_video_file)
    total_reward_dummy = 0.0  # 여기서는 리워드 합산을 따로 쓰지 않음
    print(f"Eval on {eval_video_file} -> Accuracy: {accuracy_eval:.3f}")
    all_accuracies.append(accuracy_eval)
    all_rewards.append(total_reward_dummy)
...
if all_accuracies:
    ...
    mean_FWT = np.mean(all_FWT) if all_FWT else 0.0
    std_FWT  = np.std(all_FWT)  if all_FWT else 0.0
    mean_BWT = np.mean(all_BWT) if all_BWT else 0.0
    std_BWT  = np.std(all_BWT)  if all_BWT else 0.0
    mean_IM  = np.mean(all_IM)  if all_IM else 0.0
    std_IM   = np.std(all_IM)   if all_IM else 0.0

    print(f"Average Accuracy(on eval_video_file): {mean_accuracy:.3f} (Std: {std_accuracy:.3f})")
    print(f"Average Total Reward (dummy): {mean_reward:.3f} (Std: {std_reward:.3f})")
    print(f"Average FWT: {mean_FWT:.4f} (Std: {std_FWT:.4f})")
    print(f"Average BWT: {mean_BWT:.4f} (Std: {std_BWT:.4f})")
    print(f"Average IM : {mean_IM:.4f} (Std: {std_IM:.4f})")
