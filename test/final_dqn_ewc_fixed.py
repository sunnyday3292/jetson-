...
def build_performance_matrix(agent_factory, train_order, task_frames_map, train_datasets, device, epochs_per_task=25, use_ewc=True):
    ...
    for i, ti in enumerate(train_order, start=1):
        ...
        # 누적 학습 권장
        agent.train_offline(s, a, r, ns, d, epochs=epochs_per_task, beta=5, keep_buffer=False)
...
s6, a6, r6, ns6, d6 = load_or_create(DATA_FILE_T6, video_files_t3)
...
task_frames_map = {
    ...
    "task3": frames_for_task(video_files_t6, max_frames=None),
}
...
for i in range(num_runs):
    ...
    print("\n--- Training on Task 4 ---")
    agent.train_offline(s4, a4, r4, ns4, d4, epochs=25, beta=5, keep_buffer=False)
    agent.save_params_and_fisher(list(zip(s4, a4, r4, ns4, d4)), "task4")

    print("\n--- Training on Task 5 ---")
    agent.train_offline(s5, a5, r5, ns5, d5, epochs=25, beta=5, keep_buffer=False)
    agent.save_params_and_fisher(list(zip(s5, a5, r5, ns5, d5)), "task5")

    print("\n--- Training on Task 6 ---")
    agent.train_offline(s6, a6, r6, ns6, d6, epochs=25, beta=5, keep_buffer=False)
    agent.save_params_and_fisher(list(zip(s6, a6, r6, ns6, d6)), "task3")
