...
class LaneDetector:
    def __init__(self):
        self.prev_lane = 1  # 초기값: 오른쪽 차선 ✅ 추가!
        # 빨강 HSV 범위
...
class DQNAgentCaSSLe:
    ...
    def increasing_aug_level(self, epoch, total_epochs, min_level=0.05, max_level=0.5):
        # 0~1 사이로 정규화
        progress = min(epoch / (total_epochs * 0.7), 1.0)  # 70% 지점에서 max 도달
        # 선형 증가
...
for i in range(num_runs):
    ...
    print("\n--- Training on Task 5 ---")
    agent.target_net.load_state_dict(agent.policy_net.state_dict())
    agent.train_offline(s5, a5, r5, ns5, d5, epochs=50, beta=5)
